...
/** cs */
export function compress(str, mode = 9) {
  const c = $LZMAByteArrayCompressor({}, encode(str), get_mode_obj(mode));
  while ($processChunkEncode(c.chunker));
  return $toByteArray(c.output);
}
/** ce */
/** ds */
export function decompress(byte_arr) {
  const d = $LZMAByteArrayDecompressor({}, byte_arr);
  while ($processChunkDecode(d.chunker));
