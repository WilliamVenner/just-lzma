...
test_funcs.async.dec = function decompression_test(
  compressed_file,
  correct_filename,
  next
) {
...
test_funcs.sync.dec = function decompression_test_sync(
  compressed_file,
  correct_filename,
  next
) {
    ...
    (function run_test(i) {
      ...
      if (file.slice(-5) === ".lzma") {
        ...
        test_funcs[test_type].dec(
          p.join(path_to_files, file),
          p.join(path_to_files, file.slice(0, -5)),
          function next() {
            run_test(i + 1);
          }
        );
...
function help() {
  ...
  console.log(
    "  --sync     preform synchronous tests (default is asynchronous)"
  );
  console.log("");
  console.log("TESTS:");
  console.log("");
...
function run_all_tests() {
  ...
  announce(
    "Testing lzma_worker.js" + (test_type === "sync" ? "synchronously" : "")
  );
